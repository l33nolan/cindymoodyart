...
var $dismissMessage = '<p style="margin-top: -15px; font-size: 0.9rem">[Tap again to hide details]</p>';
...
// HTML template for image and info overlay,  to be added to the page with data returned from loop
        var $imageOverlay =
            '<div class="col-sm-6 col-md-4 col-0-gutter holder">\n' +
            '<div class="ot-portfolio-item">\n' +
            '<figure class="effect-border">\n' +
            '<img src="' + imagesData[i].paintingURL + '" class="img-responsive portfolio-img" />\n' +
            '<figcaption>\n' +
...
// CSS transitions for mobile and desktop
...
if (/Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent)) {
    ...
    $(function () {
        $('.img-place').on('mouseover', '.effect-border', function () {
            $(this).toggleClass('trans');
        });
        $('.img-place').on('mouseout', '.effect-border', function () {
            $(this).toggleClass('trans');
        });
    });
}
